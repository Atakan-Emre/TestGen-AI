...
class NGVGenerator(BSCGenerator):
    def __init__(self):
        super().__init__()
        self.unique_fields = {}
        self.unique_field_keywords = [
            "tekildir",
            "benzersiz",
            "unique",
            "tekil olmalıdır",
    ...
    def _analyze_scenario_line(self, line):
        """Senaryo satırını analiz et ve tekil alanları belirle"""
        try:
            # Tekil alan kontrolü
            if not any(keyword in line.lower() for keyword in self.unique_field_keywords):
                return None
                
            print(f"Tekil alan bulundu: {line}")
            ...
            # JSON alanını ve tipini belirle
            ...
            if not json_field:
                print(f"JSON alan eşleşmesi bulunamadı: {tr_text} ({en_text})")
                return None
    ...
    def _create_duplicate_value_variations(self):
        """Tekil alanlar için tekrarlı değer test senaryoları oluştur"""
        variations = []
        
        for field, field_info in self.unique_fields.items():
            if field in self.binding_mutation_blocked_fields:
                continue
            # Temel test case'i oluştur
            base_test = self._create_test_case()
            ...
            for mandatory_field, mandatory_info in self.mandatory_fields.items():
                if mandatory_field != field and mandatory_field not in self.binding_mutation_blocked_fields:
                    value = build_valid_value(self._build_field_context(mandatory_field, mandatory_info))
                    self._set_nested_value(base_test['test_data'], mandatory_field, value)
            
            # Tekrarlı değeri oluştur
            duplicate_value = build_duplicate_seed(self._build_field_context(field, field_info))
            self._set_nested_value(base_test['test_data'], field, duplicate_value)
            
            # Aynı değeri içeren ikinci test case
            duplicate_test = self._create_test_case()
            # İkinci test case için de zorunlu alanları doldur
            for mandatory_field, mandatory_info in self.mandatory_fields.items():
                if mandatory_field != field and mandatory_field not in self.binding_mutation_blocked_fields:
                    value = build_valid_value(self._build_field_context(mandatory_field, mandatory_info))
                    self._set_nested_value(duplicate_test['test_data'], mandatory_field, value)
            # Aynı tekrarlı değeri ikinci test case'e ekle
            self._set_nested_value(duplicate_test['test_data'], field, duplicate_value)
            
            variations.append({
                "scenario_type": "NGV",
                "description": f"Tekil alan '{field_info.get('analysis', {}).get('tr_text', field)}' için tekrarlı değer testi",
                "test_data": [base_test['test_data'], duplicate_test['test_data']],
                "expected_result": "VALIDATION_ERROR",
                "expected_message": f"Alan '{field}' için tekrarlı değer hatası: {duplicate_value}"
            })
    ...
    def _load_semantic_unique_fields(self, scenario_path):
        ...
        for profile in profiles:
            ...
            if profile.required:
                self.mandatory_fields[resolved_path] = field_payload
            if profile.unique:
                self.unique_fields[resolved_path] = field_payload

        return bool(self.unique_fields)
    ...
    def generate_ngv_tests(self, scenario_path, test_name, json_file_id):
        """NGV test senaryolarını oluştur"""
        try:
            ...
            if self.binding_ignored_fields:
                ...
                self.unique_fields = {
                    field: info for field, info in self.unique_fields.items()
                    if field not in self.binding_ignored_fields
                }

            if not self._load_semantic_unique_fields(scenario_path):
                ...
                for line in lines:
                    if any(keyword in line.lower() for keyword in self.unique_field_keywords):
                        print(f"\nTekil alan satırı bulundu: {line.strip()}")
                        analysis = self._analyze_scenario_line(line)
                        if analysis and analysis.get('json_field'):
                            self.unique_fields[analysis['json_field']] = {
                                'json_field': analysis['json_field'],
                                'analysis': analysis,
                                'type': analysis['type']
                            }
                            print(f"Tekil alan eklendi: {analysis['json_field']}")
            ...
            print(f"\nToplam {len(self.unique_fields)} tekil alan bulundu")
            
            # Test varyasyonlarını oluştur
            variations = self._create_duplicate_value_variations()
            
            if not variations:
                print("Tekil alan bulunamadı veya test senaryosu oluşturulamadı.")
                return None
            ...
            output_dir = os.path.join("/app/data/output/test_cases", test_name, "ngv")
            os.makedirs(output_dir, exist_ok=True)
            
            saved_tests = []
            for i, test_case in enumerate(variations):
                filepath = os.path.join(output_dir, f"ngv_test_case_{timestamp}_{i+1}.json")
            ...
            print(f"\nTekil alan sayısı: {len(self.unique_fields)}")
            return saved_tests
            
        except Exception as e:
            print(f"NGV test üretme hatası: {str(e)}")
            raise
