...
class NGVTestRequest(BaseModel):
    scenario_id: str
    test_name: str
    json_files: List[int]
...
async def generate_ngv_test(request: NGVTestRequest):
    """NGV test senaryosu oluşturur"""
    try:
        logger.debug(f"NGV test oluşturma isteği alındı: {request}")
        
        # Test klasörünü oluştur
        test_dir = os.path.join(TEST_CASES_DIR, request.test_name, "ngv")
        os.makedirs(test_dir, exist_ok=True)
        logger.debug(f"NGV test dizini oluşturuldu: {test_dir}")
        ...
        return {
            "message": "NGV test senaryosu oluşturuldu",
            "test_name": request.test_name,
            "result": result
        }
        ...
        logger.error("NGV test oluşturma hatası", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/list/{test_name}")
async def list_ngv_tests(test_name: str):
    """Belirli bir test için NGV senaryolarını listeler"""
    try:
        test_dir = os.path.join(TEST_CASES_DIR, test_name, "ngv")
        return list_test_case_directory(test_dir)
        
    except Exception as e:
        logger.error(f"NGV test listeleme hatası: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e)) 
